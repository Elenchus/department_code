...
for (matrix, name) in [(sums, "sum"), (avgs, "average")]:
    ...
    graph_utils.create_scatter_plot(logger, Y, labels, f"MCE {code_type} replacement k-meanspatients {name} test. Silhoutte score {s}%", f'mce_{code_type}_kmeans_{name}')
    ...
    graph_utils.create_scatter_plot(logger, Y, labels, f"MCE {code_type} BGMM patients {name} test", f'mce_hip_bgmm_{name}')
